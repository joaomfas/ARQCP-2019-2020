...
######################################################
	...
	movl 8(%ebp), %ebx		#Resultados
	movl 12(%ebp), %ecx		#n_carros
	movl $0, %esi 			#volta vencedor
	movl $0, %edi			#Result vencedor
	movl $0, %eax			#Volta vencedor
	...
	ver_vencedor:
		...
		cont1:
			pushl %ecx
			pushl %edi
			movl TEMPO_TOTAL_OFFSET(%edx), %ecx
			movl TEMPO_TOTAL_OFFSET(%edi), %edi
			cmpl %ecx, %edi
			jl cont
			movl %edx, %esi
			movl %eax, %edi
