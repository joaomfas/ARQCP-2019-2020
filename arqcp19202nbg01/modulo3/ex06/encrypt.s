...
######################################################
	...
	movl $0, %eax	#Reset %eax (counter)
	
	encrypt_loop:
		movb (%esi), %cl	#Copy char from ptr1 to %cl
		cmpb $'a', %cl		#Check if the char is 'a'
		je no_changes			#No encryption if it is
		cmpb $' ', %cl		#Check if the char is ' '
		je no_changes			#No encryption if it is
		cmpb $0, %cl		#Check if the end of the string was reached
		je loop_end				#Jump to end if it was
		addb $2, (%esi)		#Add 2 to char
		addl $1, %esi		#Move to next char (char 1 byte)
		addl $1, %eax		#Increase changes counter
		jmp encrypt_loop	#Next iteration
