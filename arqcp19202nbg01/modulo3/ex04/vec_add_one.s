...
.section .text # section identifier : code
	.global vec_add_one # vois vec_add_one(void)
...
######################################################
	...
	movl ptrvec, %esi	#Copy ptr1 address to %esi
	movl num, %edx		#Move counter to %edx
	
	vec_loop:
		cmpl $0, %edx		#Checks if the counter is 0
		je vec_loop_end			#Jump to end if it is
		addl $1, (%esi)		#Add 1 to current number
		addl $4,%esi		#Move to the next number in ptrvec (int 4 bytes)
		subl $1, %edx		#Decrease counter
		jmp vec_loop		#Next iteration
