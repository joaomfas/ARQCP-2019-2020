...
######################################################
	...
	movl $0, %ecx		#Clean register ecx
 	movl $0, %edx		#Clean register edx
 	
 	movsbl A, %ebx		#Move A value to bl (8 bits)
 	movswl B, %ecx		#Move B value to cx (16 bits)
 	movl C, %eax		#Move C value to eax (32 bits)
 	movl D, %edx		#Move D value to edx (32 bits)
 	
 	addl %ebx, %eax		#C = C + A
 	addl %ecx, %eax		#C = C + B. Se houver carry, adiciona a eax.
 	subl %edx, %eax		#C = C - D
 	cltd
